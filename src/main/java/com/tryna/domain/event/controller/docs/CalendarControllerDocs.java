...
public interface CalendarControllerDocs {
    ...
    @Operation(
            summary = "B101 캘린더 메인/월간 화면 조회",
            description = "월간 캘린더 날짜별 미리보기와 선택 날짜의 일정 목록을 함께 조회합니다. B102 월간 캘린더 조회 기능은 본 API에 통합되었습니다.",
    ...
    ApiResponse<CalendarMainResponse> getCalendarMain(
            ...
            @Parameter(description = "선택 날짜(yyyy-MM-dd)", required = true, example = "2026-07-17")
            @RequestParam String selectedDate
    );

    @Operation(
            summary = "B103 날짜별 일정 목록 조회",
            description = "선택 날짜에 표시할 일정 목록을 조회합니다.",
