...
public class ExternalCalendarController implements ExternalCalendarControllerDocs {
    ...
    public ResponseEntity<ApiResponse<Void>> syncCalendar(
        ...
        calendarSyncService.syncGoogleCalendar(userId);
    ...
    public ResponseEntity<ApiResponse<ExternalCalendarStatusResponse>> getCalendarStatus(
            @AuthenticationPrincipal Long userId
    ) {
        ExternalCalendarStatusResponse response = calendarSyncService.getCalendarStatus(userId);
